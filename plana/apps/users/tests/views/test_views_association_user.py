...
class AssociationUserViewsTests(TestCase):
    ...
    def test_anonymous_post_association_user_wrong_group(self):
        ...
        """
        GroupInstitutionFundUser.objects.filter(user_id=self.unvalidated_user_id, group_id=5).delete()
        response_anonymous = self.anonymous_client.post(
            "/users/associations/",
            {
                "user": self.unvalidated_user_name,
    ...
    def test_student_post_association_user(self):
        ...
        user_asso = AssociationUser.objects.get(user_id=self.student_user_id, association_id=5)
    ...
    def test_anonymous_patch_association_user(self):
        ...
        - An anonymous user cannot execute this request.
        """
        response_anonymous = self.anonymous_client.patch(f"/users/{self.unvalidated_user_id}/associations/2")
        self.assertEqual(response_anonymous.status_code, status.HTTP_401_UNAUTHORIZED)
