...
class UserQuerySet(models.QuerySet):
    ...
    def managed_users(self, user):
        ...
        # Only Users with verified email address are returned and considered active through the API
        # Incomplete Users are not returned (at least one gifu needed)
        base_filters = Q(emailaddress__verified=True) & ~Q(groupinstitutionfunduser__isnull=True)
        ...
        if managed_institution_acronyms.exists():
            ...
            additional_filters |= Q(associations__institution__acronym__in=managed_institution_acronyms)
            ...
            if Setting.get_setting("CAS_INSTITUTION_ACRONYM") in managed_institution_acronyms:
                additional_filters |= Q(socialaccount__provider=CASProvider.id)

        # Retrieve student misc users if manager misc
        if user.has_perm("users.view_user_misc"):
            additional_filters |= Q(groupinstitutionfunduser__group__name="STUDENT_MISC")

        # A staff User cannot access to another staff User data
        restricted_scope = additional_filters & Q(is_staff=False) & base_filters
